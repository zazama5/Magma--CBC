...
// Сервер
void run_server(){
    std::array<uint32_t,8> keyArray = readKeys("keys.bin");
    ...
    while(true){
        ...
        std::thread([client_fd, plaintext, keyArray](){
...
void run_client(const std::string& server_ip, const std::array<uint32_t,8>& keyArray){
    ...
    if(packet.size() < 8){ std::cerr << "Invalid packet\n"; close(sock); return; }

    std::vector<uint8_t> ivBytes(packet.begin(), packet.begin() + 8);
    std::vector<uint8_t> ciphertext(packet.begin() + 8, packet.end());

    uint64_t iv64 = bytesToIv(ivBytes);
    std::vector<uint64_t> encryptedBlocks = splitBlocksForDecrypt(ciphertext);
    std::vector<uint64_t> decryptedBlocks = decryptCBC(encryptedBlocks, iv64, keyArray);
